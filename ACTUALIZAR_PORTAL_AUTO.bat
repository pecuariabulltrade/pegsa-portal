...
rem Ejecutar pipeline de datos
cmd /c echo.| "%PYTHON%" "%ORIGEN%\actualizar_datos.py"
if errorlevel 1 (
    >"%STATUS_FILE%" echo ERROR: python fallo %date% %time%
    if exist "%LOCK_FILE%" del "%LOCK_FILE%" >nul 2>&1
    exit /b 1
)
...
rem SIEMPRE pushear — puede haber commits previos pendientes
git push
if errorlevel 1 (
