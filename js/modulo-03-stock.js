...
function renderProductivo(data) {
  ...
  // v15.67: años base de los teóricos, para rotularlos en pantalla
  var baseAnios = ((data.meta || {}).base_anios || []).join(' y ');
  var clampTol  = (data.meta || {}).clamp_tolerancia;
  var clampTxt  = '\u00b1' + (clampTol != null ? Math.round(clampTol*100) : 25) + '%';
  if (cats90.length) {
    ...
    sec90.innerHTML =
      ...
      +'<span class="section-sub">ADP observado vs teórico por categoría · rango controlado '+clampTxt
    ...
    cats90.forEach(function(cat) {
      ...
      // Barra progreso: obs relativo al rango del clamp [teo*0.75, teo*1.25]
      // (antes usaba ±15% mientras los números de abajo mostraban ±25%: el
      //  punto quedaba en una escala distinta a la de sus propias etiquetas)
      var _tol    = (clampTol != null ? clampTol : 0.25);
      var barMin  = teo ? teo * (1 - _tol) : 0;
      var barMax  = teo ? teo * (1 + _tol) : 2;
      ...
      card.innerHTML =
        ...
        +'<span style="color:rgba(26,22,18,.45)">'+clampTxt+' teo.</span>'
        ...
        // Sub-nota cabezas + de dónde salió el teórico
        ...
        +(d.teorico_fuente ? '<div style="font-family:\'DM Mono\',monospace;font-size:10px;color:rgba(26,22,18,.3);margin-top:2px">Teórico: '+d.teorico_fuente+'</div>' : '');
    ...
    nota.innerHTML = '📐 <strong style="color:rgba(26,22,18,.65)">Metodología:</strong> ADP observado = promedio ponderado por cabezas de <em>AdpSinDebaste</em> en egresos por VENTA de los últimos 90 días · '
      +'<strong style="color:rgba(26,22,18,.65)">Teórico</strong> = la misma ventana de 90 días'+(baseAnios ? ' de '+baseAnios : ' de los dos años anteriores')+', calculado sobre egresos reales (ya no la tabla del Excel). '
      +'Si esa ventana no junta cabezas suficientes, cae al promedio anual de esos mismos años; la tabla queda como último recurso · '
      +'Rango controlado: el valor usado en la estimación de masa histórica se clampea a '+clampTxt+' del teórico.';
  ...
  if (cats.length) {
    ...
    secCat.innerHTML = '<div class="section-header"><span class="section-title">Por Categoría</span>'
      +'<span class="section-sub">ADP y estadía promedio · ventas último año vs teórico'+(baseAnios ? ' ('+baseAnios+' completo)' : '')+'</span></div>';
    var tbl = document.createElement('table'); tbl.className = 'data-table';
    tbl.innerHTML = '<thead><tr>'
      +'<th>Categoría</th>'
      +'<th class="right">Cabezas</th>'
      +'<th class="right">ADP Prom.</th>'
      +'<th class="right">ADP Teór.</th>'
    ...
    cats.forEach(function(cat) {
      ...
      // v15.67: variación contra el teórico de la base histórica
      var vCat      = d.variacion_pct;
      var varCatStr = (vCat != null) ? ((vCat >= 0 ? '+' : '') + vCat.toFixed(1) + '%') : '—';
      var varCatCol = (vCat == null) ? 'rgba(26,22,18,.3)'
                      : (Math.abs(vCat) <= 10 ? '#27613d' : (Math.abs(vCat) <= 25 ? '#b8922a' : '#c0392b'));
      var tr = document.createElement('tr');
      tr.innerHTML = '<td><strong>'+cat+'</strong></td>'
        ...
        +'<td class="right mono" style="color:rgba(26,22,18,.5)"'+(d.teorico_fuente ? ' title="'+d.teorico_fuente+'"' : '')+'>'+(d.adp_teorico != null ? fmtD(d.adp_teorico,3) : '—')+'</td>'
        +'<td class="right mono" style="font-weight:600;color:'+varCatCol+'">'+varCatStr+'</td>'
