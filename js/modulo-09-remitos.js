...
function remInformePDF() {
  ...
  var h = '<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">'
    ...
    // ...y el valor baja a 16 px sin partirse: a ~115 px de ancho útil, un
    // "$ 130.000.000" a 19 px se iba a dos líneas.
    + '.re .kc{padding:6px 8px}.re .kc .v{font-size:16px;white-space:nowrap}'
    ...
    // válvula de seguridad: con 13 columnas la tabla entra en A4 con ~11 px de
    // sobra. Si algún remito trae un nombre de tropa más largo, que envuelva esa
    // columna en vez de que la tabla desborde la hoja (table-layout:auto encoge
    // primero la única celda que puede partirse).
    + '.dt .trp{white-space:normal;word-break:break-word}'
...
/* v15.71.3 · Paleta compartida del modulo. La usa renderRemitos y tambien
   modulo-10-acumulados.js, para no tener la misma tira de estilos dos veces. */
var REM_STYLES = {
  LBL:  'font-family:\'DM Mono\',monospace;font-size:10px;letter-spacing:.12em;text-transform:uppercase;color:rgba(26,22,18,.45);margin-bottom:5px',
  VAL:  'font-family:\'Playfair Display\',serif;font-size:21px;font-weight:700',
  UNI:  'font-family:\'DM Mono\',monospace;font-size:11px;color:rgba(26,22,18,.5);margin-top:3px',
  CARD: 'background:#fff;border:1px solid var(--border);border-radius:2px;padding:13px 16px',
  GRID: 'display:grid;grid-template-columns:repeat(auto-fit,minmax(178px,1fr));gap:12px;margin-bottom:8px',
  H2:   'font-family:\'Playfair Display\',serif;font-size:20px;font-weight:700;margin:26px 0 12px',
  SUB:  'font-family:\'DM Mono\',monospace;font-size:11px;color:rgba(26,22,18,.45);margin:-6px 0 12px',
...
function renderRemitos(soloResultado) {
  ...
  // v15.71.3: la paleta vive en REM_STYLES (global, arriba) — el modulo 11 ·
  // Resultados Acumulados la usa desde ahi en vez de duplicarla.
  var LBL = REM_STYLES.LBL, VAL = REM_STYLES.VAL, UNI = REM_STYLES.UNI;
  var CARD = REM_STYLES.CARD, GRID = REM_STYLES.GRID, H2 = REM_STYLES.H2;
  var SUB = REM_STYLES.SUB, WARN = REM_STYLES.WARN, INP = REM_STYLES.INP;
  ...
  h += '<div style="' + SUB + ';margin:0 0 16px">Carga local en este navegador — se migrará a base de datos.'
    + (_remHist && _remHist.meta && _remHist.meta.n_ventas
        ? ' · Historial: <strong>' + _remHist.meta.n_ventas + ' venta'
          ...
          // v15.71.3: los informes acumulados se mudaron al modulo 11
          + ' <a onclick="sbNavigate(\'acumulados\')" style="cursor:pointer;color:var(--gold);'
          + 'text-decoration:underline">&#8594; ver Resultados acumulados</a>'
        : '')
    + '</div>';
